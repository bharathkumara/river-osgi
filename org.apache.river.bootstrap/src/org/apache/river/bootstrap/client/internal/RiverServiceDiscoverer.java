...
import org.apache.river.bootstrap.client.RiverServiceDiscoveryListener;
...
class RiverServiceDiscoverer implements ServiceDiscoveryListener, RiverService {
	private final static Logger logger = BootstrapLogger.getRiverServiceDiscovererLogger();
	...
	public RiverServiceDiscoverer(BundleContext context, ServiceDiscoveryManager discoveryManager, Bundle bundle,
		...
		logger.info("River Service discovery is constructed {}", this);
	}

	public void start() {
		logger.info("Initiating the River discovery process for the template {} ", this);
		try {
			...
			logger.info("Failed to start the River discovery process for the service template {}", this);
			logger.error("River service discovery process is failed to start", e);
		}
	}

	public void stop() {
		logger.info("Stopping the River discovery process for the template {} ", this);
		...
		for (ServiceID serviceID : keySet) {
			try {
				...
				logger.info("Failed to stop the River discovery process for the service template {}", this);
				logger.error("River service discovery process is failed to stop", e);
			}
		}
	}
	...
	public void serviceAdded(ServiceDiscoveryEvent event) {
		logger.info("River service is found for the template {} ", this);
		logger.debug("Started processing the discovered river service. ServiceTemplate =  {} ", this);
		...
		try {
			...
			if (serviceDiscoveryListener != null) {
				...
				try {
					...
					logger.info("Failed to register the River service {} into OSGI registry", serviceItem);
					logger.error("Failed to register the river service into OSGI registry", e);
				}
			}
			logger.info("River service is found Service item = {} ", serviceItem);
			ServiceID serviceID = serviceItem.serviceID;
			Object service = serviceItem.service;
			Entry[] attributeSets = serviceItem.attributeSets;
			logger.debug("Notify RiverServiceDiscoveryListener for  serviceItem = {} ", serviceItem);
			notifyListeners(serviceID, service, attributeSets, SERVICE_ADD);
		} catch (Exception e) {
			logger.info("Failed to process the new River service for the service template {} ", this);
			logger.error("Failed to process the new River service", e);
			return;
		} finally {
			currentThread.setContextClassLoader(oldCL);
		}

		logger.debug("Completed the processing of the discovered river service. ServiceTemplate =  {} ", this);
	...
	public void serviceRemoved(ServiceDiscoveryEvent event) {
		logger.info("River service is removed from the network. Template {} ", this);
		logger.debug("Started processing the discarded river service. ServiceTemplate =  {} ", this);
		Thread currentThread = Thread.currentThread();
		ClassLoader oldCL = currentThread.getContextClassLoader();
		currentThread.setContextClassLoader(bundleClassLoader);
		try {
			...
			logger.info("Failed to process the discarded river service for the service template {} ", this);
			logger.error("Failed to process the discard the river service", e);
		} finally {
			currentThread.setContextClassLoader(oldCL);
		}
	...
	private void registerNetworkService(ServiceDiscoveryEvent event) {
		ServiceItem serviceItem = event.getPostEventServiceItem();
		logger.info("Trying to register the new river service to OSGI registry ServiceItem = {} ", serviceItem);
		ServiceID serviceID = serviceItem.serviceID;
		Object service = serviceItem.service;
		...
		logger.info("Completed the sevice registration for the new river service Service Registration= {} ",
				registration);
	...
	private void unregisterNetworkService(ServiceDiscoveryEvent event) {
		ServiceItem serviceItem = event.getPreEventServiceItem();
		ServiceID serviceID = serviceItem.serviceID;
		logger.info("Trying to unregister the discarded river service from OSGI registry, ServiceItem = {} ",
				serviceItem);
