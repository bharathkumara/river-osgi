...
public class RiverServiceExporter extends ServiceTracker<Object, ServiceRegistration<ServiceItem>>
	...
	private final static Logger logger = BootstrapLogger.getRiverServiceExporterLogger();
	private ServiceIDManager idManager;
	Map<ServiceRegistration<ServiceItem>, ServiceItem> map = new IdentityHashMap<>();

	public RiverServiceExporter(BundleContext context, Filter filter, ServiceIDManager idManager) {
	...
	public ServiceRegistration<ServiceItem> addingService(ServiceReference<Object> reference) {
		...
		logger.info("Checking the OSGI service for river service properties {} ", reference);
		if (!(service instanceof Remote)) {
			logger.debug("OSGI service is not a remote object. Ignoring this service to export", reference);
			logger.info("OSGI service is not a river service {} ", reference);
			return null;
		}
		String property = (String) reference.getProperty(Constants.REMOTE_SERVICE_EXPORT_CONFIG_KEY);
		if (property == null) {
			logger.debug("OSGI service is not a having the property {} . Ignoring this service to export",
		...
		if (exporterKey != null && exporterKey instanceof String) {
			logger.debug("River Exporter property is found. Checking for the requested exporter ",
					Constants.REMOTE_SERVICE_EXPORT_CONFIG_KEY);
			if (exporterKey.equals(Constants.REMOTE_SERVICE_JERI_NO_EXPORTER)) {
				logger.debug("River Exporter property is {}. ", Constants.REMOTE_SERVICE_JERI_NO_EXPORTER);
				logger.debug("No Exporter will be created. OSGI service object will be exported");
				serviceItemProperties.put(Constants.INTERNAL_PROP_SERVICE_EXPORTER, null);
			} else {
				logger.debug("River Exporter property is {}. ", exporterKey);
				exporter = getExporter(serviceProperties);
				serviceItemProperties.put(Constants.INTERNAL_PROP_SERVICE_EXPORTER, exporter);
				logger.debug("Exporter will be created. OSGI service object will be exported using exporter {}",
						exporter);
			}
		...
		if (serviceID == null) {
			...
			idManager.putServiceID(rootPath, serviceID);
		} else {
			serviceItemProperties.put(Constants.SERVICE_ID_KEY, serviceID);
		}
		try {
			...
			logger.info("OSGI service is exported as a river service {} ", item);
			ServiceRegistration<ServiceItem> serviceRegistration = context.registerService(ServiceItem.class, item,
					serviceItemProperties);
			logger.info("Registering the river service in osgi registry {} ", serviceRegistration);
			synchronized (map) {
				map.put(serviceRegistration, item);
			}
			return serviceRegistration;
		} catch (ExportException e) {
			logger.error("Failed to export OSGI service as a river service {} ", e);
			return null;
		}
	}
	...
	public void removedService(ServiceReference<Object> reference, ServiceRegistration<ServiceItem> service) {
		logger.info("Unregistering the river service registration {} ", service);
		Exporter exporter = (Exporter) service.getReference().getProperty(Constants.INTERNAL_PROP_SERVICE_EXPORTER);
		logger.info("Unexport the river service {} using the exporter ", exporter);
		if (exporter != null) {
		...
		service.unregister();
		logger.info("Completed the unregistration of river service from osgi registry", exporter);
